...
BOOL
CTranslatorRelcacheToDXL::IsSortedMCVGenerateSortedDValues
	(
...
{
	if (!CMDTypeGenericGPDB::IsTextRelatedType(pmdtype->MDId()))
	{
		return true;
	}

	Oid typid = CMDIdGPDB::CastMdid(pmdtype->MDId())->Oid();
	...
	for (ULONG ul = 0; ul < ulNumMCVValues; ul++)
	{
		...
		double currDValue = gpdb::ConvertTextToScalar(datumMCV, typid);
...
{
	if(!IsSortedMCVGenerateSortedDValues(md_type, mcv_values, num_mcv_values))
	{
